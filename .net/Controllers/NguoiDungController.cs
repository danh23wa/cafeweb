    ...
    public class NguoiDungController : ControllerBase
    {
        ...
        public async Task<IActionResult> Register([FromBody] RegisterRequest request)
        {
            if (string.IsNullOrEmpty(request.TenNguoiDung) || string.IsNullOrEmpty(request.Email) || string.IsNullOrEmpty(request.MatKhau))
                return BadRequest(new { error = "T Historicên người dùng, email và mật khẩu là bắt buộc" });
        ...
        // Đăng nhập (chỉ dùng email và mật khẩu, không trả token)
        [HttpPost("login")]
        public async Task<IActionResult> Login([FromBody] LoginRequest request)
        {
            ...
            try
            {
                ...
                // Trả về thông tin user mà không tạo token
                return Ok(new
                {
                    user.MaNguoiDung,
                    user.TenNguoiDung,
