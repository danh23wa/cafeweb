...
const CategorySection = () => {
  ...
  // Lấy sản phẩm theo loại sản phẩm đã chọn
  useEffect(() => {
    const fetchProducts = async () => {
      if (selectedCategory?.MaLoaiSanPham) {
        try {
          const response = await axios.get(
            `http://localhost:3000/api/sanpham/category/${selectedCategory.MaLoaiSanPham}`,
            {
              params: { lang: i18n.language }, // Gửi ngôn ngữ hiện tại
            }
          );
          setProducts((prev) => ({ ...prev, [selectedCategory.TenLoaiSanPham]: response.data }));
        } catch (error) {
  ...
  const addToCart = async (product) => {
    try {
      const response = await axios.post('http://localhost:3000/api/giohang', {
