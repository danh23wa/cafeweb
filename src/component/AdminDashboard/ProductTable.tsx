...
const ProductTable = ({ products, setProducts }) => {
  ...
  const handleAddProduct = async () => {
    try {
      ...
      const res = await axios.post("http://localhost:3000/api/sanpham", productToAdd);
  ...
  const handleUpdateProduct = async () => {
    try {
      ...
      await axios.put(`http://localhost:3000/api/sanpham/${selectedProduct.MaSanPham}`, productToUpdate);
      setProducts((prevProducts) =>
        prevProducts.map((p) => (p.MaSanPham === selectedProduct.MaSanPham ? selectedProduct : p))
      );
  ...
  const handleDeleteProduct = async (MaSanPham) => {
    try {
      await axios.delete(`http://localhost:3000/api/sanpham/${MaSanPham}`);
