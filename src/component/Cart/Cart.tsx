...
const Cart = () => {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/api/giohang/${maKhachHang}`);
  ...
  const removeItem = async (maSanPham) => {
    try {
      const response = await axios.delete(`http://localhost:3000/api/giohang/${maKhachHang}/${maSanPham}`);
      if (response.status === 200) {
        const newCart = cart.filter((item) => item.MaSanPham !== maSanPham);
        setCart(newCart);
        alert(t('removeItemSuccess')); // Thêm key 'removeItemSuccess'
      }
    } catch (error) {
      console.error("Error removing item:", error);
      alert(t('removeItemError')); // Thêm key 'removeItemError'
    }
  };
  ...
  const handleCheckout = async () => {
    ...
    if (!shippingAddress.trim()) {
      alert(t('enterShippingAddress')); // Thêm key
      return;
    }

    try {
      const donHangResponse = await axios.post("http://localhost:3000/api/donhang", {
      ...
      await Promise.all(
        cart.map((item) =>
          axios.post("http://localhost:3000/api/chitiethoadon", {
            MaDonHang: MaDonHang,
      ...
      const thanhToanResponse = await axios.post("http://localhost:3000/api/thanhtoan", {
        MaDonHang: MaDonHang,
      ...
      await Promise.all(
        cart.map((item) => axios.delete(`http://localhost:3000/api/giohang/${maKhachHang}/${item.MaSanPham}`))
      );
      ...
      alert(t('checkoutError')); // Thêm key
    }
  };
