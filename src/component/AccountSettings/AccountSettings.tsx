...
const AccountSettings = () => {
  ...
  const [formData, setFormData] = useState({ TenNguoiDung: "", SoDienThoai: "", Email: "" });
  const [showChangePassword, setShowChangePassword] = useState(false);
  const [passwordData, setPasswordData] = useState({ MatKhauCu: "", MatKhauMoi: "" });
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`http://localhost:3000/api/nguoidung/${user.MaNguoiDung}`, formData);
  ...
  const handleChangePassword = async () => {
    try {
      const response = await axios.put(`http://localhost:3000/api/nguoidung/${user.MaNguoiDung}/password`, passwordData);
      alert(response.data);
